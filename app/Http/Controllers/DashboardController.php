...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $ventasPorSemana = Venta::select(DB::raw('DATENAME(dw, created_at) as dia'), DB::raw('SUM(total) as total'))
                            ->whereBetween('created_at', ["{$inicioSemana} 00:00:00", "{$finSemana} 23:59:59"])
                            ->groupBy(DB::raw('DATENAME(dw, created_at)'))
                            ;


        return view('dashboard', compact('venta', 'ventasPorSemana', 'totales'));
    }
}
